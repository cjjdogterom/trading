...
from strategy import Executor, RiskLimits, position_size, trend_from_closes
...
BARS_LOOKBACK_DAYS = 110            # genoeg voor SMA50 + ATR
...
def run_cycle(broker, db, executor, limits, universe) -> None:
    """Eén handelscyclus: kandidaten bepalen en de besten kopen."""
    if not executor.preflight():
        log.warning("Kill-switch actief — geen nieuwe entries deze cyclus.")
        return

    # Gebatchte data: één bars-call + één latest-trade-call voor het hele universum.
    ...
    # Kandidaten met confluentie, gerangschikt op gecombineerde score.
    candidates = []
    for sym in universe:
        ...
        s = sentiment.get(sym, {})
        sent, conf = float(s.get("score", 0.0)), float(s.get("confidence", 0.0))
        if trend < TREND_MIN or sent < SENTIMENT_MIN or conf < limits.min_confidence:
            continue
        price = float(getattr(trades.get(sym), "price", 0) or last_close)
        atr = _atr_from_bars(sym_bars, limits.atr_period)
        combined = W_TREND * trend + W_SENT * sent
        candidates.append((combined, sym, price, atr, trend, sent))
    candidates.sort(reverse=True)

    slots = limits.max_open_positions - executor._open_count
    log.info("Cyclus: %d kandidaten, %d vrije posities, equity $%.0f.",
             len(candidates), max(0, slots), executor._equity)

    bought = 0
    for combined, sym, price, atr, trend, sent in candidates:
        if slots <= 0:
            break
        if sym in executor._open_symbols or price <= 0:
            continue
        qty = position_size(executor._equity, price, limits.max_position_pct)
        if qty < 1:
            continue
        # ATR-trail in $, begrensd op [min_trail_pct, max_trail_pct] van de koers.
        trail = atr * limits.atr_mult
        trail = max(price * limits.min_trail_pct / 100, min(trail, price * limits.max_trail_pct / 100))
        trail_price = round(trail, 2)
        if executor.execute:
            executor.buy(sym, qty, price, trail_price=trail_price)
        else:
            log.info("[advisory] zou %s x%s @ %.2f kopen, trail $%.2f (score %.2f)",
                     sym, qty, price, trail_price, combined)
        slots -= 1
        bought += 1

    _write_snapshot(db, mode=executor.mode)
    log.info("Cyclus klaar: %d %s.", bought, "orders" if executor.execute else "advies-koopjes")
